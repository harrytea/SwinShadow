...
from tool.ucfdata import TestDataset
...
def main():
    for ee in range(10000, 10001, 5000):
        args.save_dir ='./results/' +"ucf/" +str(ee)+"lowft"
        ...
        for i, (batch, file_path) in enumerate(tqdm(test_dataloader)):
            with torch.no_grad():
                ...
                image = Image.open(os.path.join(ucf_image, file_path[0])).convert('RGB')
