...
from tool.istddata import TestDataset
...
parser.add_argument('--ckpt_dir',   default='./istd/ckpt', help='directory for checkpoints')
...
def main():
    for ee in range(9200, 9201, 5000):
        args.save_dir ='./results/' +"istd/" +str(ee)+"lowft"
        ...
        for i, (batch, file_path) in enumerate(tqdm(test_dataloader)):
            with torch.no_grad():
                ...
                image = Image.open(os.path.join(istd_image, file_path[0])).convert('RGB')
