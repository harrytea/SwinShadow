...
class Two_WindowAttention(nn.Module):
...
class Double_window_Double_attn(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size > 0:
            ...
            attn_shift = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
            attn_shift = attn_shift.masked_fill(attn_shift != 0, float(-100.0)).masked_fill(attn_shift == 0, float(0.0))

        self.register_buffer("attn_mask", attn_mask)
        self.register_buffer("attn_shift", attn_shift)
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        qkv = self.qkv(x)
        qkv = qkv.view(B*3, H, W, C)


        qkv_noshift = qkv[:, : ,:, :C//2].reshape(3, B, H, W, -1)
        qkv_shift = qkv[:, : ,:, C//2:].reshape(3, B, H, W, -1)

        # cyclic shift
        if self.shift_size > 0:
            qkv_shift = torch.roll(qkv_shift, shifts=(-self.shift_size, -self.shift_size), dims=(2, 3))

        qn, kn, vn = self.get_window_qkv(qkv_noshift)
        qs, ks, vs = self.get_window_qkv(qkv_shift)



        x1 = self.attn[0](qn, kn, vn, self.attn_mask)
        x2 = self.attn[1](qs, ks, vs, self.attn_shift)
...
if __name__ == '__main__':
    a = Double_window_Double_attn(128, (96, 96), 4, 24, 12)
    input = torch.randn((2, 96*96, 128))
    output = a(input)
    print()